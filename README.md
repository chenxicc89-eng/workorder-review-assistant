...
- **多文件智能配对**：学习中心可一次选择 Word、Excel、PDF、PNG/JPG 等资料。系统逐文件识别，优先按正文中的工单编号归并；识别不到时按文件名归并。Excel 工单台账会按表头逐行读取，不再把整张大表提交给 AI，因此不受整表 10 万字符限制。建议文件名同时包含工单编号和“工单内容 / 回单 / 不计入考核报告”等资料类型。
